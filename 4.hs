...
parser = makeMap <$> some (some (oneOf "XMAS") <* newline)
    where makeMap = Map.fromList . concat . zipWith (\y -> map (\(x, c) -> ((x, y), c))) [0..] . map (zip [0..])

part1 :: Input -> Int
part1 = length . findOccurances "XMAS"

findOccurances :: String -> Map (Int, Int) Char -> [(((Int, Int), (Int, Int)), String)]
findOccurances word grid = filter (isPrefixOf word . snd) $ allPaths grid
    where allPaths grid = (zip <*> map (pathInDir grid)) ((,) <$> directions <*> Map.keys grid)
          directions  = [(x, y) | x<-[-1..1], y<-[-1..1], x/=0 || y/=0]
          pathInDir  grid (d, p) | p `Map.member` grid = (grid ! p) : pathInDir grid (d, p + d)
                                 | otherwise = []
